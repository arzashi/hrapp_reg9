...
header('Content-Disposition: attachment; filename="รายงานข้อมูลรายงานการปฏิบัติงาน.xls"');#ชื่อไฟล์
?>
...
<center><h3 id="headreport">แบบรายงานการปฏิบัติงาน</h3></center><br>
...
<TABLE  x:str BORDER="1" style="font-size: 12px;" id="mainTable">
        ...
        <thead>
			...
			<tr>
				...
				<th width="2%" style="text-align:center; ">เลขที่อนุมัติเดินทาง</th>
			...
			if (isset($records) && count($records) > 0)
			{
				...
				foreach ($records as $record) {
					echo "<tr><td>{$index}</td><td>{$record->code}</td><td>{$record->gender}{$record->name}   {$record->surname}</td>"
						...
						."<td>{$record->docno}</td><td>{$record->Work_type_code}  {$record->Work_type_desc}</td>"
						."<td>{$record->detail}</td><td>{$record->location}</td><td>{$record->startdate}</td><td>{$record->enddate}</td>"
						."<td>{$record->sumWorkReport}</td><td>{$record->approve_user}  {$record->approve_job}</td>"
						."</tr>";
					$index++;
				}
			}
